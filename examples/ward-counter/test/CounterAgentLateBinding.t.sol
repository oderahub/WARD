...
contract CounterAgentLateBindingTest is Test {
    ...
    function test_bump_succeeds_when_policyId_unset() public {
        ...
        // wardGuarded layer short-circuits while unbound.
        vm.prank(deployer);
        agent.bump(7);
    ...
    function test_reset_reverts_with_WardRejected_when_policy_authorizes_only_bump() public {
        ...
        // reset is NOT in the policy → the wardGuarded modifier surfaces
        // the oracle denial as the typed WardRejected error. The revert
        // IS the deny-path proof — no in-contract catch-and-emit needed.
        vm.prank(deployer);
        vm.expectRevert(
            abi.encodeWithSelector(WardAgentBase.WardRejected.selector, bytes32("SELECTOR_NOT_ALLOWED"))
        );
        agent.reset();
        assertEq(counter.value(), 3, "reset blocked by policy");
    }

    function test_bump_reverts_with_WardRejected_when_target_not_allowed() public {
        ...
        // The wardGuarded modifier surfaces oracle denials as the typed
        // WardRejected error from WardAgentBase.
        vm.prank(deployer);
        vm.expectRevert(
            abi.encodeWithSelector(WardAgentBase.WardRejected.selector, bytes32("TARGET_NOT_ALLOWED"))
        );
        agent.bump(1);
    }
    ...
    function test_PolicyBound_event_emitted_on_every_set() public {
        ...
        emit WardAgentBase.PolicyBound(newId, bytes32(0), deployer);
        vm.prank(deployer);
        agent.setPolicyId(newId);

        bytes32 nextId = bytes32(uint256(0x5678));
        vm.expectEmit(true, true, true, true, address(agent));
        emit WardAgentBase.PolicyBound(nextId, newId, deployer);
        vm.prank(deployer);
        agent.setPolicyId(nextId);

        // unbind also emits with the previous id as oldPolicyId
        vm.expectEmit(true, true, true, true, address(agent));
        emit WardAgentBase.PolicyBound(bytes32(0), nextId, deployer);
        vm.prank(deployer);
        agent.setPolicyId(bytes32(0));
    }
    ...
    function test_constructor_rejects_zero_owner() public {
        vm.expectRevert(WardAgentBase.ZeroOwner.selector);
        new CounterAgent(oracle, counter, address(0));
    }

    // ---------- operator allow-list (Solidity layer, on TOP of Ward) ----------
    ...
    function test_bump_reverts_with_NotOperator_for_unauthorized_caller() public {
        ...
        // wardGuarded layer can run. POLICY_ID is still 0x0 here so this
    ...
    function test_addOperator_by_owner_lets_grantee_bump() public {
        ...
        // bob can now bump; passes the Ward layer because POLICY_ID is unset.
        vm.prank(bob);
        agent.bump(4);
    ...
    function test_modifier_order_caller_check_runs_before_oracle() public {
        ...
        // `onlyOperator wardGuarded(...)`, the cheap Solidity check fires
        // first and the revert is `NotOperator`. If a regression reorders to
        // `wardGuarded(...) onlyOperator`, the oracle's
