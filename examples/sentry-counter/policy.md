# Sentry Counter e2e policy

Entrypoint-policy model: `target` is the **agent** address (the contract that
inherits `SentryAgentBase`), and `selectors` enumerates the agent's own
...
`reset()` reverts with `SentryRejected("SELECTOR_NOT_ALLOWED")`.
...
Note on caller identity: this policy does NOT restrict `msg.sender` — Sentry
