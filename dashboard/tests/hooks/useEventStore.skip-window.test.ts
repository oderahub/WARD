 ...
 * value matters because Shannon (the L2 this dashboard targets) is
 ...
 * re-scan. A 24-hour window at Shannon's 0.5s block time
...
describe("useEventStore — skip-window constants", () => {
  it("APPROX_24H_BLOCKS encodes 24h at Shannon's 0.5s block time", () => {
    expect(APPROX_24H_BLOCKS).toBe(172_800n);
  });

  it("APPROX_24H_BLOCKS is small enough to fit inside Shannon's current ~57h history (~5M blocks)", () => {
  ...
  it("APPROX_30D_BLOCKS is still exported for forward-compat", () => {
    ...
    // Shannon has >30 days of history, but no longer drives the Skip
