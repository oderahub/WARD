 ...
 * dispatch authorization split in SentryQueue.dispatch is the only source of
...
describe("publish/SelectorRow.TIER_DESCRIPTIONS", () => {
  ...
  it("DELAYED wording reflects asker-dispatchable timer + commit-window deadline (matches SentryQueue._checkDispatchAuthorized else-branch and PastDeadline guard)", () => {
    const text = TIER_DESCRIPTIONS.DELAYED.toLowerCase();
    expect(text).toMatch(/delayseconds/);
    expect(text).toMatch(/dispatch/);
    // Commit window: SentryQueue.COMMIT_WINDOW_SECONDS = 7 days. The copy
  ...
  it("VETO_REQUIRED wording reflects owner-only active dispatch + commit-window deadline (matches SentryQueue._checkDispatchAuthorized VETO branch and PastDeadline guard)", () => {
    ...
    // Must NOT promise indefinite hold — SentryQueue.COMMIT_WINDOW_SECONDS
