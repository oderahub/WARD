/**
 * Pure helpers for chunking RPC log-scan ranges on Somnia Shannon.
 *
 * Shannon's RPC caps `eth_getLogs` at 1000 blocks per call (the node returns
 ...
 * so an off-by-one in any caller never trips the Shannon cap. Every other
...
/**
 * Shannon's eth_getLogs hard cap is 1000 blocks. We chunk at 999 to keep a
 ...
 * toBlock), so callers passing the Shannon cap of 999 stay strictly under
