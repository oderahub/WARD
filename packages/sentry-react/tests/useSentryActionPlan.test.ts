...
import { planSentryAction } from "../src/useSentryActionPlan.js";
import type { EvalIntent, EvalPolicy } from "@sentry-somnia/sdk";
...
const { preflight, REASON_CODES } = await import("@sentry-somnia/sdk");
...
describe("planSentryAction", () => {
  ...
  it("returns write-now when preflight allows", async () => {
    ...
    await expect(
      planSentryAction({ source: localSource(), intent, spentTodayWei: 0n }),
  ...
  it.each([
    ...
    const result = await planSentryAction({ source: localSource(), intent, spentTodayWei: 0n });
  ...
  it("returns DELAYED queue plan with delaySeconds parsed from local policy", async () => {
    ...
    await expect(
      planSentryAction({ source: localSource(1, 900), intent, spentTodayWei: 0n }),
  ...
  it("returns VETO_REQUIRED queue plan with delaySeconds parsed from local policy", async () => {
    ...
    await expect(
      planSentryAction({ source: localSource(2, 3600), intent, spentTodayWei: 0n }),
    ).resolves.toMatchObject({
      kind: "queue",
      tier: "VETO_REQUIRED",
      delaySeconds: 3600,
...
`;
    ...
    await expect(
      planSentryAction({ source: { kind: "spec", yaml }, intent, spentTodayWei: 0n }),
  ...
  it("awaits lazy spentTodayWei before preflight", async () => {
    ...
    await planSentryAction({ source: localSource(), intent, spentTodayWei });
