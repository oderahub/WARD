...
describe("scaffold — greenfield", () => {
  ...
  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), "create-sentry-agent-"));
  });
  ...
  it("MyAgent.sol derives from SentryAgentBase and gates via the sentryGuarded modifier", () => {
    const res = scaffold({ name: "my-agent", template: "greenfield", cwd: dir });
    const sol = readFileSync(join(res.projectRoot, "src/MyAgent.sol"), "utf8");
    expect(sol).toContain("is SentryAgentBase");
    // The greenfield entrypoint should be modifier-gated, not gated through
    // an internal `_gate(...)` helper. The modifier is now 2-arg —
    // `sentryGuarded(bytes4 selector, uint256 value)` — and checks the AGENT's
    ...
    expect(sol).toContain("sentryGuarded(");
    ...
    expect(sol).not.toMatch(/sentryGuarded\(\s*target\s*,/);
    expect(sol).not.toContain("_gate(");
    expect(sol).not.toContain("abi.encodeCall(");
    // Sanity: the file should import the abstract base too.
    expect(sol).toMatch(/import\s+["']sentry-somnia\/integration\/SentryAgentBase\.sol["']/);
  });
  ...
  it("foundry.toml has [profile.default] and the sentry-somnia remapping", () => {
    const res = scaffold({ name: "my-agent", template: "greenfield", cwd: dir });
    const toml = readFileSync(join(res.projectRoot, "foundry.toml"), "utf8");
    expect(toml).toContain("[profile.default]");
    expect(toml).toContain('"sentry-somnia/=sentry-src/"');
  });
  ...
  it("nextStepsBlock surfaces the dir name and pnpm sentry push", () => {
    const res = scaffold({ name: "my-agent", template: "greenfield", cwd: dir });
    const next = nextStepsBlock(res);
    expect(next).toContain("cd my-agent");
    expect(next).toContain("pnpm sentry push ./POLICY.md --label my-agent");
  });
});
...
describe("scaffold — counter-fixture", () => {
  ...
  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), "create-sentry-agent-cf-"));
  });
  ...
  it("produces a Counter.sol target and a SentryAgentBase-derived agent with both entrypoints modifier-gated", () => {
    ...
    expect(agent).toContain("is SentryAgentBase");
    ...
    expect(agent).toContain("sentryGuarded(this.bump.selector");
    expect(agent).toContain("sentryGuarded(this.reset.selector");
  ...
  it("scaffolded counter-fixture agent stacks an onlyOperator Solidity ACL on top of sentryGuarded", () => {
    const res = scaffold({ name: "my-counter", template: "counter-fixture", cwd: dir });
    const agent = readFileSync(join(res.projectRoot, "src/MyCounter.sol"), "utf8");
    ...
    // sentryGuarded so the cheap Solidity check fires before the oracle
    // call. Pin the exact left-to-right ordering on both entrypoints.
    expect(agent).toContain("onlyOperator sentryGuarded(this.bump.selector");
    expect(agent).toContain("onlyOperator sentryGuarded(this.reset.selector");
  });
