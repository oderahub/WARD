...
const { preflight } = await import("@ward/sdk");
...
describe("createWardGuardedWrite", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("toggles pending true then false for an allowed write", async () => {
    ...
    const write = createWardGuardedWrite({
      config: {},
      source,
  ...
  it("passes Ward metadata into the Intent builder", async () => {
    allow();
    const write = createWardGuardedWrite({
      config: {},
      source,
