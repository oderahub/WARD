...
export interface PlanWardActionArgs {
  source: PreflightSource;
  intent: EvalIntent;
  spentTodayWei: bigint | (() => Promise<bigint>);
}

export interface UseWardActionPlanOptions extends PlanWardActionArgs {
  /** Defaults true. Set false when the caller wants refresh-only planning. */
  enabled?: boolean;
}

export interface UseWardActionPlanReturn {
...
function policyForQueuePlan(source: PreflightSource): EvalPolicy {
  ...
  throw new Error(
    "useWardActionPlan: queue tier requires local or spec policy data; source.kind=chain only returns the decision.",
  );
}

function queuePlanFromPolicy(source: PreflightSource, intent: EvalIntent): Extract<WardActionPlan, { kind: "queue" }> {
  ...
  throw new Error(
    `useWardActionPlan: preflight returned a queue reason but policy tier is ${tier ?? "missing"}.`,
  );
}

export async function planWardAction(args: PlanWardActionArgs): Promise<WardActionPlan> {
  const spentTodayWei =
  ...
  return {
    ...
    reasonText: formatWardUserMessage(decision.reason),
  };
}

export function useWardActionPlan(opts: UseWardActionPlanOptions): UseWardActionPlanReturn {
  const [plan, setPlan] = useState<WardActionPlan | null>(null);
  ...
  const refresh = useCallback(async () => {
    ...
    setError(null);
    try {
      const next = await planWardAction(opts);
