...
# Ward sources (\`ln -s ../../contracts/src ward-src\` from inside this
# directory if you cloned the monorepo, OR drop them in by hand if you're
# standalone), these remappings let you \`import "ward/WardOracle.sol"\`.
remappings = [
    "ward/=ward-src/",
...
/// @notice A starter Ward-gated agent. Derives from \`WardAgentBase\`,
///         which gives you:
///           - immutable \`oracle\`
///           - mutable \`POLICY_ID\` storage slot (late-binding pattern)
///           - \`owner\` + \`onlyOwner\` + \`transferOwnership\`
///           - \`wardGuarded\`  (modifier — checks the oracle + reserves spend)
///           - \`_wardCheck\`   (lower-level: reverts if the oracle denies)
...
///         \`_wardCheck\` + the call directly.
contract ${contractName} is WardAgentBase {
    ...
    ///         constraints, drop down to \`_wardCheck\` with the full calldata.
...
/// @notice The downstream target the agent dispatches to. Has no Ward
...
import {WardOracle} from "ward/WardOracle.sol";
...
contract Deploy is Script {
    function run() external {
        ...
        WardOracle oracle = WardOracle(vm.envAddress("WARD_ORACLE"));
...
Entrypoint-policy model: the \`wardGuarded\` modifier on the agent checks
its OWN selector against the oracle. That means this policy's \`target\` is
the AGENT address (the contract that inherits \`WardAgentBase\`), and the
...
Customize:
  ...
  - Set \`dailySpendWeiCap\` to a real number once any selector accepts native
    value; the base contract enforces it via \`_wardSpentToday()\`.
...
# If you're inside the ward monorepo:
ln -s ../../contracts/src ward-src
# Otherwise, copy the Ward sources into ./ward-src/ by hand.
...
\`\`\`bash
pnpm ward push ./POLICY.md --label ${ctx.dirName}
...
every \`tryDispatch\` consults WardOracle and reverts with the rejection
...
/// @notice Counter-shaped Ward agent stacking TWO orthogonal access-control
///         layers (entrypoint-policy model):
///
///           1. \`onlyOperator\` — plain Solidity caller allow-list. Ward has
...
///           2. \`wardGuarded\` — the Ward policy gate. Asks the oracle
...
///         \`WardRejected("SELECTOR_NOT_ALLOWED")\` from the Ward layer; an
...
contract ${contractName} is WardAgentBase {
    ...
    function bump(uint256 by) external onlyOperator wardGuarded(this.bump.selector, 0) {
        counter.bump(by);
    }

    function reset() external onlyOperator wardGuarded(this.reset.selector, 0) {
        counter.reset();
    }
}
`;
...
/// @notice Minimal counter target — two selectors, no Ward awareness.
contract ${targetName} {
...
that inherits \`WardAgentBase\`), and the \`selectors\` list enumerates the
agent's own entrypoints. \`bump\` is authorized; \`reset\` is omitted so the
deny path reverts with \`WardRejected("SELECTOR_NOT_ALLOWED")\` — the
wardGuarded modifier surfaces the oracle denial as the typed revert; no
...
is independent of the Ward policy — Ward has no visibility into
...
Counter-fixture variant scaffolded by \`create-ward-agent\`. Same shape
as \`examples/ward-counter\`: stacks two orthogonal access-control layers
on \`WardAgentBase\`:
...
2. \`wardGuarded\` — the Ward policy gate. Asks the oracle whether the
...
Modifier order is \`onlyOperator wardGuarded(...)\` so the cheap caller
...
\`\`\`bash
forge install foundry-rs/forge-std
ln -s ../../contracts/src ward-src   # monorepo users only
forge build
\`\`\`
