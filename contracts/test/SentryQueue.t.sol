...
/// @notice Unit coverage for SentryQueue. Each test pins one invariant.
contract SentryQueueTest is Test {
    ...
    function test_enqueue_rejects_illegal_intent() public {
        ...
        vm.prank(asker);
        vm.expectRevert(abi.encodeWithSelector(SentryQueue.NotQueueable.selector, bytes32("TARGET_NOT_ALLOWED")));
    ...
    function test_enqueue_DELAYED_happy_path() public {
        ...
        SentryQueue.QueuedIntent memory q = queue.getRecord(execId);
        ...
        assertEq(uint8(q.state), uint8(SentryQueue.State.Pending));
    }
    ...
    function test_dispatch_DELAYED_reverts_when_caller_is_not_asker() public {
        ...
        skip(61);
        vm.prank(stranger);
        vm.expectRevert(SentryQueue.NotAuthorizedDispatcher.selector);
        queue.dispatch(execId);
    }
    ...
    function test_dispatch_DELAYED_happy_path_marks_committed_and_returns_intent() public {
        ...
        vm.expectEmit(true, true, true, true, address(queue));
        emit SentryQueue.Dispatched(execId, asker, policyId, expectedIntentHash);
    ...
    function test_dispatch_reverts_PastDeadline() public {
        ...
        vm.prank(asker);
        vm.expectRevert(SentryQueue.PastDeadline.selector);
        queue.dispatch(execId);
    }
    ...
    function test_dispatch_VETO_REQUIRED_reverts_when_caller_is_not_policyOwner() public {
        ...
        vm.expectRevert(SentryQueue.NotPolicyOwner.selector);
        queue.dispatch(execId);
    }
    ...
    function test_dispatch_reverts_PolicyChanged_PAUSED() public {
        ...
        skip(61);
        vm.prank(asker);
        vm.expectRevert(abi.encodeWithSelector(SentryQueue.PolicyChanged.selector, bytes32("PAUSED")));
        queue.dispatch(execId);
    }
    ...
    function test_dispatch_reverts_PolicyChanged_EXPIRED() public {
        ...
        skip(61);
        vm.prank(asker);
        vm.expectRevert(abi.encodeWithSelector(SentryQueue.PolicyChanged.selector, bytes32("EXPIRED")));
        queue.dispatch(execId);
    }
    ...
    function test_veto_happy_path_marks_vetoed() public {
        ...
        assertEq(uint8(queue.getRecord(execId).state), uint8(SentryQueue.State.Vetoed));
    }
    ...
    function test_getRecordHeader_omits_intent_data_keeps_fixed_intent_fields() public {
        vm.prank(asker);
        uint256 execId = queue.enqueue(policyId, _intent(address(target), SEL_PING), 0);
        SentryQueue.RecordHeader memory h = queue.getRecordHeader(execId);
        assertEq(h.policyId, policyId);
        assertEq(h.asker, asker);
        assertEq(h.tier, TIER_DELAYED);
        assertEq(uint8(h.state), uint8(SentryQueue.State.Pending));
    ...
    function test_approve_rejects_non_owner_and_non_veto_tier() public {
        ...
        vm.prank(alice);
        vm.expectRevert(SentryQueue.NotVetoTier.selector);
