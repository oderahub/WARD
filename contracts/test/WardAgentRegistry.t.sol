...
contract WardAgentRegistryTest is Test {
    WardAgentRegistry reg;
    address squatter = address(0x5);
    address realOwner = address(0x6);

    function setUp() public { reg = new WardAgentRegistry(); }
    ...
    function test_claim_rejects_non_controller() public {
        ...
        _register(agent, squatter);
        vm.prank(squatter);
        vm.expectRevert(WardAgentRegistry.NotAgentController.selector);
        reg.claimAgent(agent);
    }

    function test_claim_reverts_for_unregistered_agent() public {
        vm.prank(realOwner);
        vm.expectRevert(WardAgentRegistry.InvalidAgent.selector);
