...
contract SentryQueuePolicyVersionTest is Test {
    ...
    SentryOracle private oracle;
    SentryQueue private queue;
    ...
    function testDispatchRejectsPolicyBodyUpdatesAfterEnqueue() public {
        ...
        SentryQueue.RecordHeader memory header = queue.getRecordHeader(execId);
        ...
        vm.prank(asker);
        vm.expectRevert(abi.encodeWithSelector(SentryQueue.PolicyChanged.selector, bytes32("UPDATED")));
    ...
    function testDispatchStillSucceedsWhenPolicyVersionIsUnchanged() public {
        ...
        SentryQueue.RecordHeader memory header = queue.getRecordHeader(execId);
        assertEq(uint256(header.state), uint256(SentryQueue.State.Committed));
