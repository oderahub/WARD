...
/// @notice Base for agents that synchronously gate target calls through WardOracle.
abstract contract WardAgentBase {
    using WardCall for WardOracle;

    WardOracle public immutable oracle;
    bytes32 public POLICY_ID;
    address public owner;

    mapping(uint256 dayBucket => uint256 spentWei) private wardSpentByDay;
    ...
    mapping(uint256 execId => QueuedReservation) private wardReservations;
    ...
    ///      POLICY_ID == 0 keeps ungated mode; use `_wardCheck` + `_call` for downstream-call policies.
    modifier wardGuarded(bytes4 selector, uint256 value) {
        if (POLICY_ID != bytes32(0)) {
            (bool ok, bytes32 reason) =
                oracle.checkSelector(POLICY_ID, address(this), selector, value, _wardSpentToday());
            if (!ok) revert WardRejected(reason);
        }
        if (value != 0) {
            wardSpentByDay[_wardDayBucket(block.timestamp)] += value;
        }
        _;
    }

    constructor(WardOracle _oracle, address _owner) {
    ...
    function _call(address target, bytes memory data, uint256 value) internal returns (bytes memory returndata) {
        ...
        if (!success) revert WardCallFailed(out);
        return out;
    }

    /// @dev Reserve queued value into the enqueue-day bucket. No-op for zero value.
    function _wardReserveQueued(uint256 execId, uint256 value) internal {
        if (value == 0) return;
        uint64 bucket = uint64(_wardDayBucket(block.timestamp));
        wardSpentByDay[bucket] += value;
        wardReservations[execId] = QueuedReservation({dayBucket: bucket, status: 1, value: value});
    }

    /// @dev Mark a reservation consumed (kept counted) once its intent dispatches.
    function _wardConsumeQueued(uint256 execId) internal {
        QueuedReservation storage r = wardReservations[execId];
        if (r.status == 1) r.status = 2;
    }

    /// @dev Release a reservation (subtract from its original bucket) for a dead intent.
    function _wardReleaseQueued(uint256 execId) internal {
        QueuedReservation storage r = wardReservations[execId];
        if (r.status == 1) {
            wardSpentByDay[r.dayBucket] -= r.value;
            r.status = 2;
        }
    }
    ...
    function _wardSpentToday() internal view returns (uint256) {
        return wardSpentByDay[_wardDayBucket(block.timestamp)];
    }

    function _wardDayBucket(uint256 timestamp) private pure returns (uint256) {
