...
/// @title WardAgentRegistry
/// @notice Ownerless discovery registry for Ward-gated agents.
/// @dev First registrar owns updates, which prevents spam-overwrites without custody.
contract WardAgentRegistry {
    struct Agent {
        address agent;
        address registrar;
        address oracle;
    ...
    /// @dev Control = caller is the agent, or the agent's EIP-173/WardAgentBase owner().
