...
/// @notice Deploys WardOracle + WardQueue and writes the addresses to
...
contract Deploy is Script {
    function run() external returns (WardOracle oracle, WardQueue queue) {
        ...
        console2.log("WardOracle deployed at:", address(oracle));
        console2.log("WardQueue  deployed at:", address(queue));

        string memory json = "deployment";
        vm.serializeUint(json, "chainId", block.chainid);
        vm.serializeAddress(json, "wardOracle", address(oracle));
        vm.serializeAddress(json, "wardQueue", address(queue));
