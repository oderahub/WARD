...
Mathematical model of `SentryQueue.sol`'s state machine. Models only the
...
in `contracts/test/SentryQueue.t.sol`).
...
/-- Mirror of `SentryQueue.veto` (Pending → Vetoed; no-op on terminal states
