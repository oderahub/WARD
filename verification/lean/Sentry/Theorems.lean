...
Ten theorems pinning down the on-chain semantics:
  ...
  - Three mirror the `SentryQueue` state-machine timing properties.
  - Two (T9a, T9b) mirror the 2-step `transferPolicyOwnership` /
    `acceptPolicyOwnership` handoff in `SentryOracle`.
...
namespace Sentry.Ownership

/-! Minimal model of `SentryOracle.policyOwner` / `pendingPolicyOwner` and the
